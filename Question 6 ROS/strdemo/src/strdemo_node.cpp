...
int main(int argc, char **argv){
	...
	while (nh.ok()){
		if(runflag){
			importer(pointcloud,&pointcloud_form); // Lê Pontos do bagfile para a struct
			sem_init(&mutex, 0, 1);
			//==== THREADS ====//
			// Start new OS thread
			//printf("\nStarting_threads:\n");
			//printf(" Thread 1: ");
			//pthread_create(&thr[0], &attr[0], f1(&pointcloud_form), NULL);
			f1(&pointcloud_form); // Apagar quando implementar thread
			//printf(" Thread 2: ");
			//pthread_create(&thr[1], &attr[1], f2(&pointcloud_form), NULL);
			f2(&pointcloud_form); // Apagar quando implementar thread
			//printf(" Thread 3: ");
			//pthread_create(&thr[2], &attr[2], f3(pointcloud,&pointcloud_form), NULL);
			f3(pointcloud,&pointcloud_form); // Apagar quando implementar thread
			//=================//
...
void f3(sensor_msgs::PointCloud2::ConstPtr pc,struct PointCloud *ptc){
	...
	//==== Exporter ====// Escreve os Pontos da Estrutura para a saida (/output_points)
